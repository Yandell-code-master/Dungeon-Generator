...
export class DungeonCreator {
    ...
    public CreateRooms(): Room[] {
        ...
        for (const leaf of leaves) {
            ...
            */
            const minRoomWidth = Math.max(3, Math.floor(leaf.width * 0.5));
            const minRoomHeight = Math.max(3, Math.floor(leaf.height * 0.5));

            // Ancho y alto aleatorios dentro de los límites del nodo
            const roomWidth = Math.floor(Math.random() * (leaf.width - minRoomWidth)) + minRoomWidth;
            const roomHeight = Math.floor(Math.random() * (leaf.height - minRoomHeight)) + minRoomHeight;
            ...
            */
            const roomX = leaf.x + Math.floor(Math.random() * (leaf.width - roomWidth + 1));
            const roomY = leaf.y + Math.floor(Math.random() * (leaf.height - roomHeight + 1));
